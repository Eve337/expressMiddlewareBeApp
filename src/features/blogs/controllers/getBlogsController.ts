...
import { BlogDbType, BlogsWithPagination } from '../../../db';
import { mapToBlogViewModel } from '../../../utils/mappers';

export const getBlogsController = async (req: Request, res: Response<BlogsWithPagination>) => {
  ...
  const entities = (await blogsRepository.getAll(searchNameTerm, pageNumber, pageSize, sortBy, sortDirection));
  const result = { ...entities, items: entities.items.map(mapToBlogViewModel) };
  if(!entities) {
